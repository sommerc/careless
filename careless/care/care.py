...
def select_project(default_name="./careless_care.json", params=params):
    ...
    def btn_btn_new_proj_clicked(btn):
        new_proj_fn = gui_fsavename(
            title="Create CAREless project",
    ...
    def btn_btn_load_proj_clicked(btn):
        proj_fn = gui_fname(
            title="Load CAREless project", filetypes=[("JSON", "*.json")]
        )
        if len(proj_fn) == 0:
            return
        params.load(proj_fn)
    ...
    display(
        widgets.VBox(
            [
                widgets.HBox([widgets.Label("Project:"), btn_new_proj, btn_load_proj]),
                out_project,
            ]
        )
    )
...
def select_input():
    ...
    text_out_dir = widgets.Label(
        params["out_dir"], layout={"border": "1px solid black", "width": "400px"}
    )
    ...
    def gui_select_files_widget(name, key):
        ...
        out_files_low = widgets.Output(
            layout={"border": "1px solid black", "width": "500px", "min_height": "40px"}
        )
    ...
    def btn_convert_clicked(btn):
        ...
        if not check_ok:
            ...
            z_dim = get_pixel_dimensions(
                get_file_list(params["in_dir"], params["low_wc"])[0]
            ).z
            ...
            if (numpy.array(params["low_scaling"]) == 1).all():
                text_convert_repy.value = (
                    "Low quality images match high quality resolution"
                )
            else:
                text_convert_repy.value = "Low quality images are scaled up by ({}, {}, {}) in ZYX to match high quality resolution".format(
                    *params["low_scaling"]
                )
...
def gui_select_channel_widget():
    available_channels = list(
        range(
            get_pixel_dimensions(get_file_list(params["in_dir"], params["low_wc"])[0]).c
        )
    )
...
def select_patch_parameter():
    ...
    dd_n_patch_per_img = widgets.BoundedIntText(
        min=1, max=4096, step=1, value=params["n_patches_per_image"]
    )
    ...
    patch_parameter = widgets.VBox(
        [
            widgets.HBox(
                [widgets.Label("Channels", layout={"width": "200px"}), ms_channel]
            ),
            widgets.HBox(
                [
                    widgets.Label("Patch size", layout={"width": "200px"}),
                    patch_size_select,
                ]
            ),
            widgets.HBox(
                [
                    widgets.Label("#Patches / image", layout={"width": "200px"}),
                    dd_n_patch_per_img,
                ]
            ),
        ]
...
def select_probabilistic(params=params):
    dd_train_proba = widgets.Dropdown(
        options=[False, True], value=params["probabilistic"]
    )
    ...
    probab = widgets.VBox(
        [
            widgets.HBox(
                [
                    widgets.Label("Probabilistic", layout={"width": "200px"}),
                    dd_train_proba,
                ]
            ),
        ]
...
def select_train_paramter(params=params):
    ### Training epochs
    ###################
    int_train_epochs = widgets.BoundedIntText(
        min=1, max=4096, step=1, value=params["train_epochs"]
    )
    ...
    train_parameter = widgets.VBox(
        [
            widgets.HBox(
                [widgets.Label("#Epochs", layout={"width": "200px"}), int_train_epochs]
            ),
            widgets.HBox(
                [
                    widgets.Label("#Steps / epoch", layout={"width": "200px"}),
                    int_train_steps_per_epoch,
                ]
            ),
            widgets.HBox(
                [
                    widgets.Label("Batch size", layout={"width": "200px"}),
                    dd_train_batch_size,
                ]
            ),
        ]
...
def select_file_to_predict():
    ...
    text_predict_fn = widgets.Textarea(
        "", layout={"border": "1px solid black", "width": "800px", "height": "100%"}
    )
